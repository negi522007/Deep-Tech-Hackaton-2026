    ...
    <header className="sticky top-0 z-30 flex items-center gap-3 px-5 md:px-8 h-16 border-b border-white/[0.06] bg-ink/60 backdrop-blur-xl">
      ...
      <div className="ml-auto flex items-center gap-3">
        ...
        {isSupabaseConfigured() && (
          <button onClick={logout} className="grid h-9 w-9 place-items-center rounded-xl border border-white/10 bg-white/[0.03] text-steel hover:text-chalk transition">
            <LogOut size={16} />
          </button>
        )}
