...
import {
  Failure, SparePartRequest, Notification, Severity, PartUrgency, Submitter,
} from './types';
import {
...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  const addFailure = useCallback((data: {
    ...
    if (data.submitter) {
      const eq = equipmentById(data.equipment_id);
  ...
  const advanceWorkflow = useCallback((failureId: string) => {
    setWorkflowProgress((prev) => {
      ...
      const f = failures.find(x => x.id === failureId);
  ...
  return (
    <Ctx.Provider value={{
      failures, workflowProgress, partRequests, notifications,
      addFailure, advanceWorkflow, addPartRequest, markAllRead, unreadCount,
    }}>
      {children}
    </Ctx.Provider>
  );
