...
import { Scan, Sparkles, Package, Check, Cpu, Volume2, VolumeX } from 'lucide-react';
...
export default function AiScanner({
  ...
  // ── Synthèse vocale ──────────────────────────────────────────────────────
  const speak = useCallback((d: AiDiagnosis) => {
    if (!('speechSynthesis' in window)) return;
    window.speechSynthesis.cancel();

    const part = d.recommended_parts?.[0]?.name;
    const text = [
      `Diagnostic de l'équipement ${context.equipmentName ?? ''}.`,
      d.diagnosis,
      `Niveau de sévérité : ${SEVERITY_FR[d.severity] ?? d.severity}.`,
      `Confiance de l'analyse : ${Math.round(d.confidence * 100)} pourcent.`,
      part ? `Pièce recommandée : ${part}.` : '',
      d.recommended_actions?.[0] ? `Action immédiate : ${d.recommended_actions[0]}.` : '',
    ].filter(Boolean).join(' ');
  ...
  async function start() {
    ...
    if (result) {
      onDiagnosis?.(result);
      // Auto-speak after brief render delay
      setTimeout(() => speak(result), 600);
    }
  }
  ...
  useEffect(() => {
    ...
    if (boxesRef.current) {
      ...
      tl.fromTo(boxes, { opacity: 0, scale: 0.7 }, { opacity: 1, scale: 1, duration: 0.4, stagger: 0.35, ease: 'back.out(2)' }, 0.8);
    }
    return () => { tl.kill(); };
  }, [phase]);
    ...
    <div className="space-y-4">
      ...
      <div className="glass relative overflow-hidden p-2">
        <div className="relative aspect-video w-full overflow-hidden rounded-xl bg-ink2">
          ...
          {/* Voice indicator */}
          {speaking && (
            <div className="absolute right-3 top-3 flex items-center gap-1.5 rounded-full bg-cyan/20 border border-cyan/40 px-2.5 py-1 backdrop-blur">
              <Volume2 size={12} className="text-cyan animate-pulse" />
              <span className="label-mono text-cyan">lecture vocale</span>
            </div>
          )}
        </div>
      </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="ml-auto flex items-center gap-3">
                ...
                <button onClick={toggleSpeech}
                  title={speaking ? 'Arrêter la lecture' : 'Lire le diagnostic à voix haute'}
                  className={`grid h-9 w-9 place-items-center rounded-lg border transition-all ${
                    speaking ? 'border-cyan/60 bg-cyan/15 text-cyan' : 'border-[var(--hair)] text-steel hover:text-chalk'
                  }`}>
