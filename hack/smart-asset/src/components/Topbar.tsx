...
export default function Topbar() {
  ...
  function handleSearch(e: React.FormEvent) {
    ...
    const q = query.trim();
    if (q) {
      router.push(`/equipments?q=${encodeURIComponent(q)}`);
      setQuery('');
    }
  }
  ...
  return (
    <header className="sticky top-0 z-30 flex items-center gap-3 px-5 md:px-8 h-16 border-b border-[var(--hair)] bg-[var(--bg)]/60 backdrop-blur-xl">
      <form onSubmit={handleSearch} className="hidden sm:flex items-center gap-2 rounded-xl border border-[var(--hair)] bg-[var(--surface1)]/40 px-3 py-1.5 w-72 focus-within:border-cyan/40 transition-colors">
        <Search size={15} className="text-steel shrink-0" />
        <input
          value={query}
          onChange={e => setQuery(e.target.value)}
          onKeyDown={handleKeyDown}
          placeholder="Rechercher un actif, une panne…"
