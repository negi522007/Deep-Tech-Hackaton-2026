...
import { Cpu, ArrowRight, ScanLine, Boxes, Activity, ShieldCheck, Zap, Camera, Wrench } from 'lucide-react';

const STATS = [
  { v: 99.2, s: '%', label: 'Disponibilité visée' },
  { v: 10, s: ' étapes', label: 'Workflow réparation' },
  { v: 72, s: '%', label: 'Réduction délai sourcing' },
        ...
        <div className="relative z-10 mx-auto grid max-w-7xl grid-cols-1 lg:grid-cols-2 items-center gap-8 px-6 pt-6 lg:pt-16">
          ...
          <div>
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease }}
              className="inline-flex items-center gap-2 rounded-full border border-cyan/25 bg-cyan/5 px-4 py-1.5 text-sm font-medium text-cyan mb-6">
              <span className="h-2 w-2 rounded-full bg-cyan animate-pulse" /> Industrial AI · Spare Parts Intelligence
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, ease, delay: 0.25 }}
              ...
              <p className="text-xs text-steel uppercase tracking-widest">Choisissez votre accès</p>
              <div className="flex flex-wrap gap-3">
                <Link href="/technicien" className="btn btn-primary px-6 py-3 gap-2">
                  <Wrench size={16} /> Portail Technicien
                </Link>
                <Link href="/dashboard" className="btn btn-ghost px-6 py-3 gap-2">
                  <ShieldCheck size={16} /> Espace Admin
                </Link>
              </div>
              <div className="flex gap-2 pt-1">
                <Link href="/failures/new" className="text-sm text-steel hover:text-cyan transition flex items-center gap-1.5 font-medium">
                  <ScanLine size={14} /> Tester le diagnostic IA directement
                </Link>
              </div>
            </motion.div>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.92 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1, ease, delay: 0.2 }}
            ...
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 text-xs text-steel font-medium tracking-wide">glissez pour faire pivoter · turbine temps réel</div>
          </motion.div>
        </div>
      ...
      <section className="relative mx-auto max-w-5xl px-6 pb-28">
        <FadeIn>
          <div className="glass relative overflow-hidden p-10 sm:p-14 text-center">
            ...
            <Link href="/dashboard" className="relative btn btn-primary mt-8 px-8 py-3">Entrer dans AssetIQ <ArrowRight size={16} /></Link>
          </div>
        </FadeIn>
