...
type Field = {
  name: string; model: string; manufacturer: string; serial_number: string;
  category: string; company_id: string; location: string; purchase_date: string;
};
...
export default function NewEquipmentPage() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    setDone(true);
    setTimeout(() => router.push('/equipments'), 1400);
  }
  ...
  if (done) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
        ...
        <p className="text-steel text-sm">Visible dans l&apos;inventaire — redirection…</p>
      </div>
    );
  }
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <label className="space-y-1.5">
              ...
              <input value={form.name} onChange={(e) => set('name', e.target.value)}
                placeholder="ex : Convoyeur ligne 4" className="field-input" required />
            </label>
            <label className="space-y-1.5">
              <span className="text-xs text-steel">Numéro de série *</span>
              <input value={form.serial_number} onChange={(e) => set('serial_number', e.target.value)}
                placeholder="ex : CNV-042" className="field-input font-mono" required />
            </label>
          </div>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <label className="space-y-1.5">
              ...
              <input value={form.manufacturer} onChange={(e) => set('manufacturer', e.target.value)}
                placeholder="ex : SIEMENS" className="field-input" />
            </label>
            <label className="space-y-1.5">
              <span className="text-xs text-steel">Modèle</span>
              <input value={form.model} onChange={(e) => set('model', e.target.value)}
                placeholder="ex : C200" className="field-input" />
            </label>
          </div>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <label className="space-y-1.5">
              ...
              <select value={form.category} onChange={(e) => set('category', e.target.value)}
                className="field-input cursor-pointer" required>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <label className="space-y-1.5">
              ...
              <input value={form.location} onChange={(e) => set('location', e.target.value)}
                placeholder="ex : Hall A" className="field-input" />
            </label>
            <label className="space-y-1.5">
